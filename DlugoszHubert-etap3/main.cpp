...
int main(int argc, char* argv[])
{
    ...
    do {
        ...
        case 2:
            ...
            catch (MagazynInvalidPosition& ex) {
                std::cerr << ex.what();
            }
            catch (MagazynInvalidAmount& ex) {
                std::cerr << ex.what();
            }
            break;
        case 3:
...
void add(Magazyn& mag, istream& in) {
    if (mag.GetSize() == 0)                                                                                  //zabezpieczenie gdy magazyn jest pusty
        ...
        in >> q;
        in.ignore();
        mag.Add(pos-1, q);                                                                       //przekazanie do metody
    }
}
...
void remove(Magazyn& mag, istream& in) {
    if (mag.GetSize() == 0)                                                                                   //zabezpieczenie gdy magazyn jest pusty
        cout << "\nMagazyn pusty. Najpierw wstaw nowy produkt do magazynu. " << endl;
            ...
            in >> q;
            in.ignore();
            mag.Remove(pos-1, q);                                                                    //przekazanie do metody - metoda Remove klasy magazyn zwraca true jesli odjecie ilosci produktu sie powiodlo, false - jesli sie nie powiodlo
    }
}
