#include "Magazyn.h"
#include "MagazynError.h"


Kontener<std::string> Magazyn::_sharedProducts;
Kontener<int>  Magazyn::_sharedProductsID;
...
void Magazyn::Insert(std::string name, int type_of_product) {
	...
	if (!isAlready) {
		if (type_of_product != 1 && type_of_product != 2 && type_of_product != 3) {
			throw MagazynInvalidInput("Nieprawidlowy typ produktu.\n");
			return;
		}
		...
		for (int i = 0; i < _sharedProducts.get_size(); i++) {
		...
		if (exists == true) {
			...
			_sharedProductsID.push_back(_sharedProductsID.get_size());
		}
...
bool Magazyn::Add(int pos, double q) {
	bool result = false;
	if (pos >= _size || pos < 0)
		throw MagazynInvalidPosition("Nieprawidlowa pozycja. Operacja nieudana. \n");
	else if (q < 0) {
		throw MagazynInvalidAmount("\nLiczba musi byc nieujemna. Operacja nieudana. \n");
	}
	else {
		*(_arrTowar[pos]) += q;
...
void Magazyn::ShowSharedProductsID() const {
	for (int i = 0; i < _sharedProductsID.get_size(); i++) {
